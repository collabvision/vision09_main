...
html{scroll-behavior:smooth}
body{background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;font-weight:300;overflow-x:hidden;cursor:none}
#cur{width:8px;height:8px;background:var(--cur);position:fixed;pointer-events:none;z-index:9999;transform:translate(-50%,-50%);transition:width .35s,height .35s,border-radius .35s}
#cur.big{width:44px;height:44px;background:transparent;border:1px solid var(--cur);border-radius:0}
#cur-ring{width:32px;height:32px;border:1px solid var(--cur-ring);position:fixed;pointer-events:none;z-index:9998;transform:translate(-50%,-50%);transition:width .5s,height .5s}
nav{position:fixed;top:0;left:0;right:0;display:flex;justify-content:space-between;align-items:center;padding:1.2rem clamp(1.5rem,5vw,4rem);z-index:200;border-bottom:1px solid var(--border);background:var(--nav-bg);backdrop-filter:blur(16px)}
.logo{font-family:'Playfair Display',serif;font-size:1.5rem;font-weight:700;letter-spacing:.06em;color:var(--text);text-decoration:none}
.logo .g{color:var(--accent)}
.nav-t{font-family:'Tenor Sans',sans-serif;font-size:.58rem;letter-spacing:.28em;text-transform:uppercase;color:var(--muted)}
.sec-orn{font-family:'Tenor Sans',sans-serif;font-size:.55rem;letter-spacing:.3em;text-transform:uppercase;color:var(--accent);margin-bottom:.9rem;display:flex;align-items:center;gap:.7rem}
.sec-orn::before{content:'';flex:1;max-width:40px;height:1px;background:linear-gradient(270deg,var(--accent),transparent)}
.sec-orn::after{content:'';flex:1;max-width:40px;height:1px;background:linear-gradient(90deg,var(--accent),transparent)}
.sec-rule{width:100%;height:1px;background:linear-gradient(90deg,transparent,var(--accent-m),transparent);opacity:.3}
.hero{min-height:62vh;display:flex;flex-direction:column;justify-content:center;padding:clamp(8rem,16vh,12rem) clamp(1.5rem,5vw,4rem) clamp(3rem,6vh,5rem);position:relative;overflow:hidden;background:var(--bg)}
.hero-noise{position:absolute;inset:0;opacity:.015;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='.75' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E");background-size:200px 200px;pointer-events:none}
.hero-glow{position:absolute;bottom:-30%;right:-5%;width:600px;height:600px;background:radial-gradient(ellipse,var(--accent-l) 0%,transparent 65%);opacity:.1;pointer-events:none}
.scan{position:absolute;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--scan),transparent);animation:scanDown 8s linear infinite;pointer-events:none}
@keyframes scanDown{0%{top:-2%}100%{top:102%}}
.hero-k{font-family:'Tenor Sans',sans-serif;font-size:.58rem;letter-spacing:.28em;text-transform:uppercase;color:var(--accent);margin-bottom:1.6rem;opacity:0;animation:fadeUp .7s .3s forwards;display:flex;align-items:center;gap:.8rem}
.hero-k span{width:20px;height:1px;background:var(--accent);display:inline-block}
.hero-title{font-family:'Playfair Display',serif;font-size:clamp(3rem,9vw,9.5rem);line-height:.87;letter-spacing:-.02em;overflow:hidden;font-weight:900;color:var(--text)}
.ht{display:block;clip-path:inset(0 0 100% 0);animation:clipR .9s cubic-bezier(.16,1,.3,1) forwards}
.ht1{animation-delay:.45s}.ht2{animation-delay:.6s;color:var(--accent);font-style:italic}.ht3{animation-delay:.75s}
@keyframes clipR{to{clip-path:inset(0 0 0% 0)}}
.hero-sub{max-width:520px;margin-top:2rem;font-size:clamp(.88rem,1.4vw,1rem);line-height:1.85;color:var(--text-sec);opacity:0;animation:fadeUp .8s 1s forwards}
.cgrid{padding:clamp(4rem,8vh,7rem) clamp(1.5rem,5vw,4rem);display:grid;grid-template-columns:1fr 1fr;gap:clamp(3rem,6vw,7rem);align-items:start;background:var(--bg)}
.ch{font-family:'Playfair Display',serif;font-size:clamp(1.6rem,3.2vw,2.8rem);font-weight:600;line-height:1.15;margin-bottom:clamp(2rem,4vh,3rem);color:var(--text)}
.ch em{color:var(--accent);font-style:italic}
.fw{position:relative;margin-bottom:2rem}
.fi{width:100%;background:transparent;border:none;border-bottom:1px solid var(--field-b);padding:.9rem 0;font-family:'DM Sans',sans-serif;font-size:clamp(.88rem,1.4vw,.98rem);font-weight:300;color:var(--text);outline:none;transition:border-color .3s;appearance:none}
.fi::placeholder{color:var(--muted)}
.fi:focus{border-bottom-color:var(--field-fb)}
.fi option{background:var(--card);color:var(--text)}
.fl{position:absolute;top:.9rem;left:0;font-family:'Tenor Sans',sans-serif;font-size:.56rem;letter-spacing:.22em;text-transform:uppercase;color:var(--muted);pointer-events:none;transition:top .25s,font-size .25s,color .25s}
.fi:focus~.fl,.fi:not(:placeholder-shown)~.fl{top:-.88rem;font-size:.5rem;color:var(--accent)}
.fb{position:absolute;bottom:0;left:0;height:1px;width:0;background:linear-gradient(90deg,var(--accent-d),var(--accent),var(--accent-m));transition:width .45s ease}
.fi:focus~.fb{width:100%}
.btn-sub{background:var(--btn-bg);color:var(--btn-txt);font-family:'Playfair Display',serif;font-size:.9rem;font-weight:700;letter-spacing:.18em;padding:1.1rem 3rem;border:none;cursor:pointer;display:block;width:100%;margin-top:.5rem;text-transform:uppercase;position:relative;overflow:hidden;transition:background .3s,transform .3s}
.btn-sub::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.2),transparent);transform:translateX(-100%);transition:transform .4s}
.btn-sub:hover::after{transform:translateX(100%)}
.btn-sub:hover{background:var(--btn-hover);transform:translateY(-2px)}
.btn-sub:disabled{opacity:.6;cursor:not-allowed;transform:none}
.btn-wa{display:inline-flex;align-items:center;gap:.8rem;background:transparent;color:var(--text);font-family:'Playfair Display',serif;font-size:.9rem;font-weight:600;letter-spacing:.15em;padding:1.1rem 2.5rem;border:1px solid var(--btn-ob);cursor:pointer;text-decoration:none;width:100%;justify-content:center;margin-top:1rem;text-transform:uppercase;transition:border-color .3s,color .3s,transform .3s}
.btn-wa:hover{border-color:var(--wa);color:var(--wa);transform:translateY(-2px)}
.wa-dot{width:8px;height:8px;background:var(--wa);border-radius:50%;flex-shrink:0}
.success-box{border:1px solid var(--success-border);padding:3.5rem 2.5rem;text-align:center;background:var(--success-bg);position:relative}
.success-box::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--accent),var(--accent-m),var(--accent),transparent)}
.s-title{font-family:'Playfair Display',serif;font-size:2.8rem;color:var(--success-txt);margin-bottom:.6rem;font-weight:700}
.s-sub{color:var(--text-sec);font-size:.95rem;line-height:1.8}
.err-box{border:1px solid #c0392b;background:rgba(192,57,43,.06);padding:1rem 1.5rem;margin-bottom:1.5rem;font-size:.88rem;color:#c0392b;line-height:1.6}
.drow{display:flex;align-items:center;gap:1.5rem;padding:1.2rem 0;border-bottom:1px solid var(--border-f);transition:.3s;cursor:default}
.drow:hover .dl{color:var(--accent)}
.dicon{width:40px;height:40px;border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;background:var(--card);transition:border-color .3s,background .3s}
.drow:hover .dicon{border-color:var(--accent);background:var(--bg-alt)}
.dl{font-family:'Tenor Sans',sans-serif;font-size:.56rem;letter-spacing:.2em;text-transform:uppercase;color:var(--muted);margin-bottom:.2rem;transition:color .3s;display:block}
.dv{font-size:clamp(.88rem,1.4vw,1rem);color:var(--text)}
.avail{margin-top:2rem;border:1px solid var(--border);padding:clamp(1.5rem,3vw,2.5rem);position:relative;background:var(--card)}
.avail::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--accent),transparent)}
.avail-tags{display:flex;flex-wrap:wrap;gap:.6rem;margin-top:1.2rem}
.atag{font-family:'Tenor Sans',sans-serif;font-size:.54rem;letter-spacing:.18em;text-transform:uppercase;border:1px solid var(--tag-b);color:var(--tag-t);padding:.4rem .9rem}
.map-box{margin-top:1.5rem;border:1px solid var(--border);aspect-ratio:16/7;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;background:var(--card)}
.map-grid-bg{position:absolute;inset:0;background-image:linear-gradient(var(--border-f) 1px,transparent 1px),linear-gradient(90deg,var(--border-f) 1px,transparent 1px);background-size:28px 28px}
.why{padding:clamp(4rem,8vh,7rem) clamp(1.5rem,5vw,4rem);background:var(--bg-alt)}
.wgrid{display:grid;grid-template-columns:repeat(2,1fr);gap:clamp(1rem,2vw,1.5rem);margin-top:clamp(2rem,4vh,3rem)}
.wcard{border:1px solid var(--border);padding:clamp(1.5rem,3vw,2.5rem);position:relative;overflow:hidden;background:var(--card);box-shadow:0 2px 16px var(--border-f);transition:border-color .4s,background .4s,box-shadow .4s}
.wcard::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,var(--accent-d),var(--accent),var(--accent-m));transform:scaleX(0);transform-origin:left;transition:transform .55s}
.wcard:hover::before{transform:scaleX(1)}
.wcard:hover{border-color:var(--accent-m);background:var(--bg-alt);box-shadow:0 12px 40px var(--border)}
.wcard::after{content:'';position:absolute;bottom:10px;right:10px;width:16px;height:16px;border-bottom:1px solid var(--border);border-right:1px solid var(--border)}
.wnum{font-family:'Playfair Display',serif;font-size:4rem;color:transparent;-webkit-text-stroke:1px var(--border);line-height:1;margin-bottom:.5rem;font-weight:900}
.wtitle{font-family:'Playfair Display',serif;font-size:clamp(1.1rem,1.9vw,1.5rem);letter-spacing:.03em;margin-bottom:.8rem;font-weight:600;color:var(--text)}
.wtext{font-size:clamp(.78rem,1.15vw,.9rem);line-height:1.82;color:var(--text-sec)}
.cta{padding:clamp(6rem,14vh,10rem) clamp(1.5rem,5vw,4rem);display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;overflow:hidden;background:var(--bg)}
.cta-ghost{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-family:'Playfair Display',serif;font-size:clamp(5rem,18vw,18rem);color:transparent;-webkit-text-stroke:1px var(--border);white-space:nowrap;pointer-events:none;animation:gPulse 8s ease-in-out infinite;font-weight:900}
.cta-orn{font-family:'Tenor Sans',sans-serif;font-size:.58rem;letter-spacing:.28em;text-transform:uppercase;color:var(--accent);margin-bottom:1.4rem;display:flex;align-items:center;gap:.8rem;justify-content:center;position:relative}
.cta-orn span{width:30px;height:1px;background:var(--accent);display:inline-block}
.cta-h{font-family:'Playfair Display',serif;font-size:clamp(2rem,5.2vw,4.8rem);font-weight:600;line-height:1.08;max-width:740px;position:relative;color:var(--text)}
.cta-h em{color:var(--accent);font-style:italic}
.cta-sub{margin-top:1.5rem;font-size:clamp(.85rem,1.4vw,1rem);color:var(--text-sec);max-width:500px;line-height:1.8;position:relative}
.cta-btns{margin-top:2.5rem;display:flex;gap:1.2rem;flex-wrap:wrap;justify-content:center;position:relative}
.btn-solid{background:var(--btn-bg);color:var(--btn-txt);font-family:'Playfair Display',serif;font-size:.88rem;font-weight:700;letter-spacing:.15em;padding:1rem 2.8rem;border:none;cursor:none;text-decoration:none;display:inline-block;text-transform:uppercase;position:relative;overflow:hidden;transition:background .3s,transform .3s}
.btn-solid::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.2),transparent);transform:translateX(-100%);transition:transform .4s}
.btn-solid:hover::after{transform:translateX(100%)}
.btn-solid:hover{background:var(--btn-hover);transform:translateY(-2px)}
.btn-outline{background:transparent;color:var(--btn-ot);font-family:'Playfair Display',serif;font-size:.88rem;font-weight:600;letter-spacing:.15em;padding:1rem 2.8rem;border:1px solid var(--btn-ob);cursor:none;text-decoration:none;display:inline-flex;align-items:center;gap:.7rem;text-transform:uppercase;transition:border-color .3s,color .3s,transform .3s}
.btn-outline:hover{border-color:var(--btn-ohover);color:var(--btn-ohover);transform:translateY(-2px)}
.rv{opacity:0;transform:translateY(38px);transition:opacity .85s ease,transform .85s cubic-bezier(.16,1,.3,1)}
.rv.on{opacity:1;transform:translateY(0)}
.d1{transition-delay:.1s}.d2{transition-delay:.2s}.d3{transition-delay:.3s}.d4{transition-delay:.4s}
...
export default function ContactPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        name: form.name || "Krishna",
        company: form.company || "Vision9",
        email: form.email || "mythdor10@gmail.com",
        phone: form.phone || "9876543210",
        service: form.service || "Digital Marketing",
