...
const v = (t) => [
  ...
  `--cur:${t.cursorFill}`,`--cur-ring:${t.cursorRing}`,
  `--btn-bg:${t.btnSolidBg}`,`--btn-txt:${t.btnSolidText}`,`--btn-hover:${t.btnSolidHover}`,
  `--btn-ob:${t.btnOutlineBorder}`,`--btn-ot:${t.btnOutlineText}`,`--btn-ohover:${t.btnOutlineHover}`,
].join(";")

/* ── Service data — backgrounds reference THEME directly ── */
const buildCards = (t) => [
  [
    { id: "branding", label:"BRANDING",               bg:t.svc1Bg, color:t.svc1Text, tall:true  },
    { id: "pitch-decks", label:"PITCH\nDECKS",           bg:t.svc2Bg, color:t.svc2Text, tall:true  },
    { id: "logo-design", label:"LOGO\nDESIGN",           bg:t.svc6Bg, color:t.svc6Text, tall:false },
    { id: "influencer-marketing", label:"INFLUENCER\nMARKETING",  bg:t.svc4Bg, color:t.svc4Text, tall:true  },
    { id: "packaging-design", label:"PACKAGING\nDESIGN",      bg:t.svc5Bg, color:t.svc5Text, tall:false },
    { id: "performance-marketing", label:"PERFORMANCE\nMARKETING", bg:t.svc1Bg, color:t.svc1Text, tall:true  },
  ],
  [
    { id: "brand-identity", label:"BRAND\nIDENTITY",        bg:t.svc4Bg, color:t.svc4Text, tall:true  },
    { id: "empty1", label:"",                        bg:t.svc3Bg, color:t.svc1Text, tall:false },
    { id: "social-media", label:"SOCIAL MEDIA\nMGMT",     bg:t.svc1Bg, color:t.svc1Text, tall:true  },
    { id: "empty2", label:"",                        bg:t.svc2Bg, color:t.svc2Text, tall:false },
    { id: "website-design", label:"WEBSITE\nDESIGN",        bg:t.svc6Bg, color:t.svc6Text, tall:true  },
...
function VerticalRail({ cards, direction, offset = 0, speed = 0.5, onHover, onClick, activeId, railIndex }) {
  ...
  useEffect(() => {
    ...
    const targetOffset = -((cardPos.index * cardHeight) + (cardHeight / 2) - containerHeight / 2)
  ...
  return (
    <div ref={ref} style={{ position:"absolute",top:0,left:0,right:0,display:"flex",flexDirection:"column",
      gap:GAP,willChange:"transform",transform:`translateY(${pos.current}px)` }}>
      {[...cards,...cards,...cards].map((s,i) => (
        <div 
          key={`${s.id}-${i}`}
          data-label={s.label}
          onMouseEnter={() => s.label && onHover?.(s.label)}
          onClick={() => s.label && onClick?.(s.label, railIndex, i % cards.length)}
          style={{ 
            background:s.bg, 
            ...
            cursor:"pointer",
            transform: s.label && activeId && s.label.split(" ")[0] === activeId.split(" ")[0] 
              ? "scale(1.02)" 
              : "scale(1)",
            boxShadow: s.label && activeId && s.label.split(" ")[0] === activeId.split(" ")[0]
              ? "0 10px 20px rgba(0,0,0,0.2)"
              : "none",
            zIndex: s.label && activeId && s.label.split(" ")[0] === activeId.split(" ")[0] ? 10 : 1
          }}
          onMouseOver={e => {
            if (!(s.label && activeId && s.label.split(" ")[0] === activeId.split(" ")[0])) {
...
function HorizontalRail({ cards, direction, offset = 0, speed = 0.5, onHover, onClick, activeId, railIndex }) {
  const ref = useRef(null)
  const pos = useRef(offset)
  const loop = useRef(0)
  const rafRef = useRef(null)
  ...
  useEffect(() => {
    ...
    const targetOffset = -((cardPos.index * cardWidth) + (cardWidth / 2) - containerWidth / 2)
  ...
  return (
    <div ref={ref} style={{ position:"absolute",top:0,left:0,bottom:0,display:"flex",flexDirection:"row",
      gap:GAP,willChange:"transform",transform:`translateX(${pos.current}px)` }}>
      {[...cards,...cards,...cards].map((s,i) => (
        <div 
          key={`${s.id}-${i}`}
          data-label={s.label}
...
export default function ServicesPage() {
  ...
  const handleCardClick = (label, railIndex, cardIndex) => {
    setActive(label)
    setActiveId(label)
    console.log(`Card clicked: ${label} in rail ${railIndex} at position ${cardIndex}`)
  }
  ...
  return (<>
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        {!isMobile && [{t:24,l:24,bTop:true,bLeft:true},{t:24,r:0,bTop:true,bRight:true},
          {b:24,l:24,bBottom:true,bLeft:true},{b:24,r:0,bBottom:true,bRight:true}
        ].map((c,i) => (
          <div key={i} style={{ position:"absolute", top:c.t, bottom:c.b, left:c.l, right:c.r,
            width:28, height:28, opacity:.45,
        ...
        <p style={{ 
          ...
          {isMobile ? "Tap a service to explore" : "Click a service to see it in first column"}
        </p>
        ...
        <div style={{ 
          ...
          {[...new Set([...COL1,...COL2].filter(c=>c.label).map(c => c.label.replace(/\n/g, " ")))].map((s,i) => {
            ...
            return (
              <div 
                ...
                style={{ 
                  ...
                  borderLeft: !isMobile && isActive ? `2px solid var(--accent)` : "none",
